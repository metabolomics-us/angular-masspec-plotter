...
class AngularMasspecPlotterController {
    constructor() {
         ...
         * @returns {number}
         */
        this.maxIntensityInRange = (data, min, max) => {
         ...
         * @param n
         */
        this.plotAnnotations = (data, plot, placeholder, n) => {
            // Get peaks
        ...
        this.redrawPlot = (data, plot, placeholder, showAnnotations) => {
            let mzMax = Math.max.apply(Math, data.map((x) => {
                return x[0];
            }));
            let intensityMax = Math.max.apply(Math, data.map((x) => {
